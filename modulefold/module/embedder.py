...
class InputEmbedder(torch.nn.Module):
    ...
    def one_hot(self, x, v_bins):
        ...
        (x의 각 값이 v_bins의 어떤 bin에 해당하는지 찾아 One-hot edcoding함)
        ...
        """
        x = x.unsqueeze(-1)
        p = torch.argmin(torch.abs(x - v_bins), dim=-1)     # p: (B, N, N), x에 가장 가까운 bin의 index

        p = torch.nn.functional.one_hot(p, num_classes=v_bins.shape[-1]).to(x.dtype)    
        # p: (B, N, N, 2 * replos_k + 1)
    ...
    def relpos(self, ri):
        ...
        (ri에 속한 두 residue 간의 상대적 거리를 Encoding)

        ri: (B, i), residue_index
        d: (B, i, j), 두 residue 간의 상대적 거리
    ...
    def forward(self, tf, ri, msa):
        ...
        return: m: (B, N_clust, N_res, c_m), MSA representation
                z: (B, N_res, c_z, c_z), residue pair representation
        """
        a, b = self.linear1(tf), self.linear2(tf)
        z = a[..., None, :] + b[..., None, :, :]
        #   (B, N_res, 1, c_z) + (B, 1, N_res, c_z) -> (B, N_res, N_res, c_z)
        z += self.relpos(ri)
