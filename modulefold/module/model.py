...
class Alphafold2(torch.nn.Module):
    ...
    def iteration(self, batch, prev_m, prev_z, prev_x):
        ...
        # input embedder
        m, z = self.input_embedder(batch["target_feat"].float(), batch["residue_index"].float(), batch["msa_feat"].float())
        ...
        outputs["pair"] = z
        outputs["single"] = s
        outputs["sm"] = self.structure_module(s, z, batch["aatype"], batch["seq_mask"])
        ...
        return outputs, m[..., 0, :, :], z, outputs["final_atom_positions"]
    ...
    def forward(self, batch):
        ...
        for cycle_no in range(num_iters):
            ...
            if cycle_no != num_iters - 1:
                with torch.no_grad():
                    outputs, m, z, x = self.iteration(feats, m, z, x)
            else: # last iteration
                outputs, m, z, x = self.iteration(feats, m, z, x)
