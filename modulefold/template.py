...
class TemplatePointwiseAttention(torch.nn.Module):
    ...
    def forward(self, t, z, template_mask=None):
        ...
        if q.dim() == 4:
            a = torch.einsum("i j h c, s i j h c -> s i j h", q, k) * (c_h ** -0.5) + 1e9 * (template_mask[..., None, None, None, :] - 1)
        else:
            a = torch.einsum("b i j h c, b s i j h c -> b s i j h", q, k) * (c_h ** -0.5) + 1e9 * (template_mask[..., None, None, None, None, :] - 1)
        a = torch.nn.functional.softmax(a, dim=-4) # (B, s, i, j, h)

        if a.dim() == 4:
            o = torch.einsum("s i j h, s i j h c -> i j h c", a, v) # (B, i, j, h, c)
        else:
