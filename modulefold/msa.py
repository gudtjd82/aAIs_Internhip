...
class MSARowAttentionWithPairBias(torch.nn.Module):
    def __init__(self, c_m=256, c_z=128, c_h=4, n_head=8):
        ...
        self.proj_q = torch.nn.Linear(c_m, c_h * n_head, bias=False)
        self.proj_k = torch.nn.Linear(c_m, c_h * n_head, bias=False)
        self.proj_v = torch.nn.Linear(c_m, c_h * n_head, bias=False)
        self.proj_b = torch.nn.Linear(c_z, n_head, bias=False)
        self.proj_g = torch.nn.Linear(c_m, c_h * n_head)
        self.proj_o = torch.nn.Linear(c_h * n_head, c_m)
    ...
    def forward(self, m, z, msa_mask=None):
        ...
        if q.dim() == 4:
            a = torch.einsum("s i h c, s j h c -> s i j h", q, k) / (c_h**0.5) # (B, s, i, j, h)
            if msa_mask != None:
        ...
        if a.dim() == 4:
            o = gate * torch.einsum("s i j h, s j h c -> s i h c", a, v) # (B, s, i, h, c)
        else:
